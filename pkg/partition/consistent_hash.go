...
type Node struct {
	ID string
	Address string
	VNodes []uint64 // these are the visual node tokens on the hash ring
}

// also added the network address representing in the node
// cluster

type ConsistentHash stuct {
	nodes map[string]*Node
	ring []ringEntry // sorted list of virtual node
	numVNodes int
	replica int
	mu sync.RWMutex
}
...
func (ch *ConsistentHash) AddNode(id, address string){
	...
	for i := 0; i < ch.numVNodes; i++ {
		token := ch.hash(fmt.Spritnf("%s-%d", id, i))
		node.VNodes[i] = token
...
func (ch *ConsistentHash) RemoveNode(id string){
	...
	node, exists := ch.nodes[id]
	if !exists{
		return
	}

	// todo : removing the virtual nodes from the ring
	...
	sort.Slice(ch.ring, func(i, j int) bool {
		return ch.ring[i].token < ch.ring[j]
	})
}
...
func (ch *ConsistentHash) hash(key string) uint64 {
	...
	// using the first 8 bytes for uint64
	var token uint64
